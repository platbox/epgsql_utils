...
pack_(datetime, DateTime = {_, _}) -> DateTime;
...
pack_(date, Date = {_, _, _}) -> Date;
pack_(time, Time = {_, _, _}) -> Time;
...
unpack_(datetime, {Date, Time}) -> {unpack_(date, Date), unpack_(time, Time)};
...
unpack_(date, Date = {_, _, _}) -> Date;
unpack_(time, _Time = {H, M, S}) -> {H, M, erlang:round(S)};
